...
import { TVShow, Video, Cast } from "@/types";
import {
  getTVDetails,
  getTVVideos,
  getTVCredits,
...
export default function TVWatchPage() {
  ...
  useEffect(() => {
    ...
    const fetchShowData = async () => {
      try {
        const [details, videos, credits, similarShows] = await Promise.all([
          getTVDetails(showId),
          getTVVideos(showId),
          getTVCredits(showId),
          getSimilarTV(showId),
        ...
        setSimilar(similarShows.data.results.slice(0, 5));
  ...
  if (!show) {
    return (
      <main style={{
        ...
        <p style={{ color: "#e50914", fontSize: "18px" }}>Show not found.</p>
      </main>
    );
  }

  const backdropUrl = `${IMAGE_URL}/original${show.backdrop_path}`;
      ...
      <div style={{ position: "relative", height: "70vh", width: "100%" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "16px" }}>
            ...
            <span style={{ color: "#e5e5e5" }}>{show.first_air_date?.slice(0, 4)}</span>
          </div>
